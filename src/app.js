...
import { setupEventHandlers, moveDirection, isTouchscreenDevice, touchEvent, createJoystick, onDocumentMouseMove } from './resources/eventHandlers';
...
import { simpleText, floatingLabel, createTextOnPlane } from './resources/surfaces';
...
Ammo().then((Ammo) => {
  ...
  //create floor box (texture on top)
  function createFloorBox(x, y, z, scaleX, scaleY, scaleZ, boxTexture, URLLink, color = 0x000000, transparent = true) {
    ...
    var materials = [
      ...
      loadedTexture, // Top side   ---> THIS IS THE TOP
      borderMaterial, // Bottom side
      borderMaterial, // Front side
      borderMaterial, // Back side
    ];

    const linkBox = new THREE.Mesh(new THREE.BoxGeometry(boxScale.x, boxScale.y, boxScale.z), materials);
    linkBox.position.set(x, y, z);
    linkBox.renderOrder = 1;
    linkBox.castShadow = true;
    linkBox.receiveShadow = true;
    // linkBox.userData = { URL: URLLink, email: URLLink }; // No link for now usually
    scene.add(linkBox);
    objectsWithLinks.push(linkBox.uuid);

    addRigidPhysics(linkBox, boxScale);

    // cursorHoverObjects.push(linkBox); // Hover optional
  }
  ...
  //create Ammo.js body to add solid mass to "Vivek Mesh"
  function vivekWords(x, y, z) {
  ...
  //loads text for Vivek Mesh
  function loadVivekText() {
    ...
    text_loader.load('./src/jsm/Roboto_Regular.json', function (font) {
      ...
      var geometry = new TextGeometry('Vivek Pandey', {
        font: font,
        size: 3,
        height: 0.5,
        curveSegments: 12,
  ...
  function loadEngineerText() {
    ...
    text_loader.load('./src/jsm/Roboto_Regular.json', function (font) {
      ...
      var geometry = new TextGeometry('ML & MERN DEV', {
        font: font,
        size: 1.5,
        height: 0.5,
        curveSegments: 20,
  ...
  function createBillboard(x, y, z, textureImage = billboardTextures.grassImage, urlLink, rotation = 0) {
    ...
    const billboardPole = new THREE.Mesh(
      new THREE.BoxGeometry(billboardPoleScale.x, billboardPoleScale.y, billboardPoleScale.z),
  ...
  function createTriangle(x, z) {
    ...
    // Adjusting coords to match original relative setup if needed, but simple triangle is fine.
    // Original: v1(4,0,0), v2(5,0,0), v3(4.5,1,0)
  ...
  function start() {
    ...
    // Skills Section (On the Ground)
    // Box dimensions: width 40, height 0.5, depth 30. Positioned at X=-50, Z=55 (Shifted down)
    // Texture on TOP face.
    createFloorBox(-50, 0.25, 55, 40, 0.5, 30, boxTexture.skillsChart, null, 0xffffff, false);
    // Heading flat on ground? No, heading usually stands up or floats.
    // User said "ball will hover on it smoothly". So no big obstacle.

    // Heading:
    simpleText(-50, 0.01, 35, 'TECHNICAL SKILLS', 3);

    createBillboard(-45, 2.5, -78, billboardTextures.bagHolderBetsTexture, URL.bagholderBets, Math.PI * 0.17);

    createBillboard(-17, 1.25, -75, billboardTextures.homeSweetHomeTexture, URL.homeSweetHomeURL, Math.PI * 0.15);

    // Project titles (Visible in front of billboard) and Descriptions (Below title)
    // Billboards are at Z ~ -75. We place titles at -40 to be clearly in front on the floor.
    simpleText(-70, 0.01, -40, 'Engineering Hub', 2.5);
    simpleText(-70, 0.01, -34, inputText.engineeringHubDesc, 0.75);

    // simpleText(-50, 0.01, -5, 'SKILLS', 3);
    // simpleText(-42, 0.01, -80, 'PROJECTS', 5); // Removed as per instruction
    simpleText(-42, 0.01, -45, 'Jarvis', 2.5);
    simpleText(-42, 0.01, -39, inputText.jarvisDesc, 0.75);

    simpleText(-14, 0.01, -40, 'DameChess', 2.5);
    simpleText(-14, 0.01, -34, inputText.damechessDesc, 0.75);
    ...
    createBox(19, 2, -70, 4, 4, 1, boxTexture.LinkedIn, URL.twitter, 0x0077b5, true);
    // createBox(
    //   35,
    //   2,
    //   -70,
    ...
    createBox(35, 2, -70, 4, 4, 1, boxTexture.globe, URL.devTo, 0x000000, false);
    ...
    floatingLabel(35, 6.5, -70, '   Main \nPortfolio');
    // floatingLabel(44, 6.5, -70, '   How I \nmade this');

    // Achievements Section - Title closer to content
    simpleText(61, 0.01, 15, 'ACHIEVEMENTS', 3.5);
    simpleText(61, 0.01, 20, inputText.achievements, 0.85);
    ...
    simpleText(-20, 0.01, 25, instructionsText, 1.25);

    // Professional Bio - centered in front of name
    // Professional Bio - centered in front of name
    const bioText = '> End-to-End Full Stack Development: Architecting scalable, secure web\n   ecosystems designed for commercial deployment.\n\n> Generative AI & Chatbots: Engineering intelligent, context-aware\n   conversational agents and Agentic AI solutions using LLMs.\n\n> Applied Machine Learning: Integrating predictive models and data-driven\n   algorithms into functional, production-ready applications.';
    simpleText(10, 0.01, -5, bioText, 1.0);

    simpleText(20, 0.01, 25, touchText, 1.25);

    // Contact Information - Centered and visible
    simpleText(23, 0.01, -45, 'CONTACT', 2.5);
    simpleText(23, 0.01, -50, 'Phone: +91 7887671316', 1);
    simpleText(23, 0.01, -53, 'Location: Mumbai, Maharashtra', 1);
    simpleText(23, 0.01, -56, 'Company: technewity.com', 1);
